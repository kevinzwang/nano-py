...
class AniMu(commands.Cog, name='AniMu (Anime Music)'):
    ...
    async def removelist(self, ctx, *names):
        ...
        if (game := self.games.get(ctx.guild.id)) and not game.loop.done() and set(names).issuperset(settings.anime_lists):
            return await ctx.send('You can\'t remove all of the lists while a game is active!')
